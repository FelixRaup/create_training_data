...
#define THERMAL_DUMP_ALL_RATES
! further down these two arrays are defind:
!      cool_rates,cool_rates_chem

#define UNUSED_PARAM(x)  x = x
#define ABORT(x) stop

#define DTCOOL_SCALE_FACTOR 1d-2
#define ABUND_DT 1d-1
...
! Map symbolic indices to numbers
      ...
      integer ih2, ihp, itmp, iHatom, icp, ico, ic2p
      integer ichx, iohx, ihcop, ihep, imp, ihe2p
...
#define THERMAL_DUMP_ALL_RATES
      REAL cool_rates(nrates)
      REAL cool_rates_chem(nrates_chem)
...
! These variables are initialized in photoinit
      REAL phtab(nphtab), phgam(nphtab), f_rsc
...
!
      ...
      integer no_chem, no_cooling, isrf_option

      logical useFullNewton, useAssistH2Decrease
     ...
     &               AV_ext, Z_atom, minDeltaT, crphot,                   &
     &               phgam, abundHe,                                      &
     &               cool_rates, cool_rates_chem

      common /cooli/ iphoto, iflag_mn, iflag_ad, iflag_atom               &
     &,              iflag_3bh2a, iflag_3bh2b, iflag_h3pra                &
     &,              isrf_option, no_chem, no_cooling

      common /coolb/ useFullNewton, useAssistH2Decrease
